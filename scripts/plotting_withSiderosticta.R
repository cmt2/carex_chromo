...
pdf("~/Documents/carex_chromo/convergence_withSiderosticta.pdf")
...
plot(trace1$gamma, 
     ...
     ylab = NA,
     xlab = NA,
     main = paste0("Gamma (chromosome gains) 1 ESS: ", round(ess1["gamma"], digits = 2)))
plot(trace2$gamma, 
     type = "l", 
     yaxt = 'n',
     ylab = NA,
     xlab = NA,
     main = paste0("Gamma (chromosome gains) 2 ESS: ", round(ess2["gamma"], digits = 2)))
...
plot(trace1$delta, 
     ...
     ylab = NA,
     xlab = NA,
     main = paste0("Delta (chromosome losses) 1 ESS: ", round(ess1["delta"], digits = 2)))
plot(trace2$delta, 
     type = "l", 
     yaxt = 'n',
     ylab = NA,
     xlab = NA,
     main = paste0("Delta (chromosome losses) 2 ESS: ", round(ess2["delta"], digits = 2)))
...
plot(trace1$rho, 
     ...
     ylab = NA,
     xlab = NA,
     main = paste0("Rho (polyploidization) 1 ESS: ", round(ess1["rho"], digits = 2)))
plot(trace2$rho, 
     type = "l", 
     yaxt = 'n',
     ylab = NA,
     xlab = NA,
     main = paste0("Rho (polyploidization) 2 ESS: ", round(ess2["rho"], digits = 2)))
...
p1_rho <- plotTrace(t1, vars = "rho") 
p2 <- plotTrace(t2, vars = c("gamma","delta")) 
p2_rho <- plotTrace(t2, vars = "rho") 
pc <- plotTrace(tc, vars = c("gamma","delta")) 
pc_rho <- plotTrace(tc, vars = "rho") 

grid.arrange(p1[[1]] + ggtitle("Trace 1"), p1_rho[[1]] + ggtitle("Trace 1 Rho"),
             p2[[1]] + ggtitle("Trace 2"), p2_rho[[1]] + ggtitle("Trace 2 Rho"),
             pc[[1]] + ggtitle("Combined Trace"), pc_rho[[1]] + ggtitle("Combined Trace Rho"))
